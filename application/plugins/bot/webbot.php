...
class WebBot {
	...
	private function log($message, $method) {
		...
		$file = self::$conf_store_dir.'log.txt'; // /WebBot/tmp/log.txt
	...
	public function execute() {
		...
		foreach ($this->urls as $id => $location) {
			if($this->formatUrl($location)) {
				...
				if($response->success) {
					$success++;
					$this->documents[] = new \WebBot\Document($response, $id, $location);
	...
	private function saveBody($body) {
		$file = self::$conf_store_dir.'body.txt';
		file_put_contents($file, "\t\t\t\t----------\t----------\n", FILE_APPEND);	
